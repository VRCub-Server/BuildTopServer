...
const APP_ID = process.env.NEXT_PUBLIC_APP_ID || 'YOUR_APP_ID';
const APP_KEY = process.env.NEXT_PUBLIC_APP_KEY || 'YOUR_APP_KEY';
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ getActiveWorks() {
    ...
    const activeWorks = serverBuildings.filter((building)=>building.competitionId && activeCompetitionIds.includes(building.competitionId) && building.status === '有效').sort((a, b)=>b.votes - a.votes);
    return JSON.parse(JSON.stringify(activeWorks));
}
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ getHistoricalWorks() {
    const competitions = await getCompetitions();
    const historicalCompetitionIds = competitions.filter((comp)=>comp.status !== '进行中').map((comp)=>comp.id);
    const historicalWorks = serverBuildings.filter((building)=>building.competitionId && historicalCompetitionIds.includes(building.competitionId) && building.status === '有效' // Only show '有效' works in history too
    ).sort((a, b)=>{
        if (b.eventYear !== a.eventYear) {
            return b.eventYear - a.eventYear;
        }
        return b.votes - a.votes;
    });
    return JSON.parse(JSON.stringify(historicalWorks));
}
...
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerServerReference"])(getHistoricalWorks, "00c67d8b8b918702e79a7bc06e989a177ca4ea094e", null);
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__f241be2c._.js.map